...
>    并亲自完成【整体框架搭建】与【高难度部分攻坚】（核心算法、复杂交互、数据存储等）。
> 🟢 **较低性能模型担任执行脑**：落地执行者，基于策略脑产出的 framework 继续完成剩余任务
>    （细节样式、常规组件、文案填充等），低成本、高吞吐。
> 开启🌐联网检索后，双脑均通过 DuckDuckGo / Bing / Wikimedia Commons 进行真实网络搜索，绝不依赖内部知识库。
>
